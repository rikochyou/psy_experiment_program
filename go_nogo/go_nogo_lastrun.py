...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='E:\\Project\\psychopy_fanshi\\go_nogo\\go_nogo_lastrun.py',
...
# --- Initialize components for Routine "刺激" ---
...
# --- Initialize components for Routine "空屏" ---
...
trials = data.TrialHandler(nReps=20.0, method='fullRandom', 
    ...
    trialList=data.importConditions('loop.xlsx'),
...
for thisTrial in trials:
    ...
    for thisComponent in 注视点Components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # the Routine "注视点" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    
    # --- Prepare to start Routine "刺激" ---
    continueRoutine = True
    routineForceEnded = False
    # update component parameters for each repeat
    image.setImage(pic)
    ...
    _key_resp_2_allKeys = []
    # keep track of which components have finished
    刺激Components = [image, key_resp_2]
    for thisComponent in 刺激Components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    while continueRoutine and routineTimer.getTime() < 0.5:
        ...
        if key_resp_2.status == STARTED and not waitOnFlip:
            theseKeys = key_resp_2.getKeys(keyList=['j','J'], waitRelease=False)
    ...
    # --- Prepare to start Routine "空屏" ---
    continueRoutine = True
    routineForceEnded = False
    # update component parameters for each repeat
    # keep track of which components have finished
    空屏Components = [polygon_2]
    for thisComponent in 空屏Components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
...
_key_resp_3_allKeys = []
# keep track of which components have finished
指导语2Components = [text_2, key_resp_3]
...
for thisTrial_2 in trials_2:
    ...
    _key_resp_4_allKeys = []
    # keep track of which components have finished
    刺激2Components = [image_2, key_resp_4]
    ...
    while continueRoutine and routineTimer.getTime() < 0.5:
        ...
        if key_resp_4.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > key_resp_4.tStartRefresh + 0.5-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if key_resp_4.status == STARTED and not waitOnFlip:
            theseKeys = key_resp_4.getKeys(keyList=['f','F'], waitRelease=False)
