...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='E:\\Project\\psychopy_fanshi\\IAT\\IAT_lastrun.py',
...
text = visual.TextStim(win=win, name='text',
    text='你好，欢迎参加本实验！\n本测试为一项快速分类任务，一共包含7个部分。\n需要你进行分类的词语有：\n积极词：美丽，漂亮，喜爱，愉悦\n消极词：丑陋，恶心，厌恶，难受\n花：玫瑰，牡丹，丁香，月季\n虫：蚂蚁，蜈蚣，蝗虫，蚯蚓\n请你先熟悉一下这些词语。\n为了便于你区分，在测试中：\n积极词或消极词用白色显示，花或虫用绿色显示\n每一部分测试开始前，会有详细的任务说明。\n如果你已经准备好，\n请按空格键进入第一部分的测试说明\n\n如果有任何疑问，请联系实验员',
    font='Open Sans',
...
# --- Initialize components for Routine "Block指导语" ---
...
# --- Initialize components for Routine "Block结束语" ---
...
# --- Initialize components for Routine "结束语" ---
...
for thisBlock in blocks:
    ...
    # --- Prepare to start Routine "Block指导语" ---
    continueRoutine = True
    routineForceEnded = False
    # update component parameters for each repeat
    text_3.setText(InstrText)
    key_resp_4.keys = []
    key_resp_4.rt = []
    _key_resp_4_allKeys = []
    # keep track of which components have finished
    Block指导语Components = [text_3, key_resp_4]
    for thisComponent in Block指导语Components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # the Routine "Block指导语" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    for thisTrial in trials:
        ...
        routineForceEnded = False
        # update component parameters for each repeat
        text_5.setText(stims)
        key_resp_5.keys = []
        key_resp_5.rt = []
        _key_resp_5_allKeys = []
        # keep track of which components have finished
        trialComponents = [label_left, label_right, polygon_2, text_5, key_resp_5]
    ...
    # completed reps repeats of 'trials'
    
    
    # --- Prepare to start Routine "Block结束语" ---
    continueRoutine = True
    routineForceEnded = False
    # update component parameters for each repeat
    key_resp_2.keys = []
    ...
    _key_resp_2_allKeys = []
    # keep track of which components have finished
    Block结束语Components = [text_2, key_resp_2]
    for thisComponent in Block结束语Components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # the Routine "Block结束语" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
...
# --- Prepare to start Routine "结束语" ---
continueRoutine = True
routineForceEnded = False
# update component parameters for each repeat
# keep track of which components have finished
结束语Components = [text_4]
for thisComponent in 结束语Components:
    thisComponent.tStart = None
    thisComponent.tStop = None
    thisComponent.tStartRefresh = None
    thisComponent.tStopRefresh = None
...
for thisComponent in 结束语Components:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
# the Routine "结束语" was not non-slip safe, so reset the non-slip timer
routineTimer.reset()
